...
async function readItemTableMessages(db: Database.Database): Promise<RawCursorMessage[]> {
  if (!hasTable(db, "ItemTable")) {
    return [];
  }
...
async function readCursorDiskKvMessages(db: Database.Database): Promise<RawCursorMessage[]> {
  if (!hasTable(db, "cursorDiskKV")) {
    return [];
  }
 ...
 */
async function* streamCursorDiskKvMessages(db: Database.Database): AsyncIterable<RawCursorMessage> {
...
async function readCanonicalComposers(db: Database.Database): Promise<CanonicalComposer[]> {
  const statement = db.prepare(
